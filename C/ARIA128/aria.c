...
//p_text: plain text, en_text: encrypt text(result text), len:plain text length
uint8_t EnCrypt(uint8_t *p_text, uint8_t *en_text,uint8_t len){
  ...
  if(len%16==0){
    ...
    encrypt_len=len/16+1;
  }

  memset(temp_buffer,0x00,sizeof(temp_buffer));
  memcpy(temp_buffer,p_text,len);

  EncKeySetup(master_key,rk,128);
  for (i=0;i<encrypt_len;i++){
    Crypt(temp_buffer+idx,12,rk,en_text+idx);
    idx+=16;
  }
...
//en_text : encrypt text, de_text : decrypt text(result text),len : encrpyt text length 
uint8_t DeCrypt(uint8_t *en_text,uint8_t *de_text,uint8_t len){
  ...
  DecKeySetup(master_key, rk, 128);
  for (i=0;i<decrypt_len;i++){
    Crypt(en_text+idx,12,rk,de_text+idx);
...
void ARIA_128test(){
    ...
    //test plain ! Have a good day
  uint8_t plain[28]={0x74, 0x65, 0x73, 0x74, 0x20, 0x70, 0x6c, 0x61, 0x69, 0x6e, 0x20, 0x21, 0x20, 0x48, 
                 0x61, 0x76, 0x65, 0x20, 0x61, 0x20, 0x67, 0x6f, 0x6f, 0x64, 0x20, 0x64, 0x61, 0x79}; 
  uint8_t encrypt_buffer[64];
  uint8_t decrypt_buffer[64];
  uint8_t encrypt_len=0;
  uint8_t decrypt_len=0;
  ...
  encrypt_len=EnCrypt(plain,encrypt_buffer,sizeof(plain));
  printf("encrpyt msg : ");
  for(i=0;i<encrypt_len;i++){
      printf("%x ",encrypt_buffer[i]);
  }
  printf("\r\n");
  
  printf("Decrypt msg : ");

  decrypt_len=DeCrypt(encrypt_buffer,decrypt_buffer,encrypt_len);
  for(i=0;i<decrypt_len;i++){
